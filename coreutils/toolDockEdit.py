...
class ToolDockEdit(widget.OWWidget):
    ...
    def addWidgetToCategory(self, category, directory, widgetPath, confirmation=True):
        ...
        if os.path.exists(destPath) or os.path.exists(destLink):
            ret = qm.question(
                self,
                "",
                "symlink {} or destination widget {} exists - OverWrite ?".format(
                    destLink, destPath
                ),
                qm.Yes | qm.No,
            )
    ...
    def updateCategories(self):
        ...
        for index, category in enumerate(self.categories):
            ...
            if os.path.islink(iconLink):
                ...
                self.categoryToPath[category] = "/widgets/{}".format(
                    self.categoryToDirectory[category]
                )
    ...
    def updateWidgetList(self):
        ...
        for category in self.categories:
            ...
            self.widgetList[category] = [
                os.path.basename(x)[2:-3]
                for x in glob("/biodepot/{}/OW*.py".format(directory))
            ]
    ...
    def drawAddWidget(self):
        ...
        cbox = self.makeComboBox(
            self.grid, " to drawer:", self.categories, startRow=1, startColumn=4
        )
        widgetAddBtn = gui.button(
            None, self, "Add", callback=lambda: self.widgetAdd(ledit, cbox)
        )
    ...
    def drawRenameWidget(self):
        ...
        self.RNWwbox = self.makeComboBox(
            self.grid, "Rename widget ", self.widgetList[self.categories[0]], startRow=3
        )
        self.RNWcbox = self.makeComboBox(
            self.grid,
            " in drawer ",
            self.categories,
            startRow=3,
            startColumn=4,
            callback=lambda: self.__onRWCategoryChoice(self.RNWcbox, self.RNWwbox),
        )
        self.RNWledit = self.makeLedit(
            self.grid,
            "New name",
            "to new name",
            startRow=3,
    ...
    def drawRemoveWidget(self):
        self.updateWidgetList()
        self.RWwbox = self.makeComboBox(
            self.grid, "Remove widget ", self.widgetList[self.categories[0]], startRow=2
        )
        self.RWcbox = self.makeComboBox(
            ...
            startRow=2,
            startColumn=4,
            callback=lambda: self.__onRWCategoryChoice(self.RWcbox, self.RWwbox),
        )
    ...
    def widgetRemove(self):
        ...
        if workflowPaths:
            ...
            ret = qm.question(
                self,
                "",
                "Workflows {} use this widget - Do you still want to remove it?".format(
                    workflowStr
                ),
                qm.Yes | qm.No,
            )
            ...
            for workflowPath in workflowPaths:
                workflowTools.removeWidgetfromWorkflow(
    ...
    def widgetRename(self, widgetName=None, category=None, newName=None):
        ...
        if workflowPaths:
            ...
            ret = qm.question(
                self,
                "",
                "Workflows {} use this widget - Do you still want to rename it?".format(
                    workflowStr
                ),
                qm.Yes | qm.No,
            )
            if ret == qm.No:
                return
        for workflowPath in workflowPaths:
            workflowTools.renameWidgetInWorkflow(
                workflowPath, workflowPath, category, widgetName, newName
            )
    ...
    def findWorkflowsWithWidget(self, widgetName, widgetCategory):
        ...
        for category in self.categories:
            ...
            for owsFile in owsFiles:
                print(
                    "checking " + owsFile + " for " + widgetCategory + " " + widgetName
                )
                doc = minidom.parse(owsFile)
                nodes = doc.getElementsByTagName("node")
                for node in nodes:
                    print(
                        "node pn {} node name {} category {} wname {}".format(
                            node.getAttribute("project_name"),
                    ...
                    if (
                        node.getAttribute("project_name") == widgetCategory
    ...
    def drawRemoveCategory(self):
        cbox = self.makeComboBox(
            self.grid, "Remove drawer:", self.categories, startRow=4, startColumn=1
        )
        catRemoveBtn = gui.button(
            None, self, "Remove", callback=lambda: self.categoryRemove(cbox)
        )
    ...
    def categoryRemove(self, nameCombo):
        ...
        if category not in self.categories:
            ...
            return
        ret = qm.question(
            self, "", "Remove {} from ToolDock ?".format(category), qm.Yes | qm.No
        )
        if ret == qm.No:
            return
        directory = self.categoryToDirectory[category]
        removeCategoryFromToolDock(
            self.baseToolPath, category, self.categoryToDirectory[category]
        )
        registerDirectory(self.baseToolPath)
        self.updateCategories()
        if self.canvas.recent_schemes:
            currentDirectory = os.path.basename(
                os.path.dirname(self.canvas.recent_schemes[0][1])
            )
    ...
    def getCategoryList(self, widgetName):
        ...
        for category in self.categories:
            directory = self.categoryToDirectory[category]
            if os.path.exists(
                "{}/{}/OW{}.py".format(self.baseToolPath, directory, widgetName)
            ):
