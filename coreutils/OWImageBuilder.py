...
class OWImageBuilder(widget.OWWidget):
    ...
    def __init__(self, dockerDir):
        ...
        fontTitle.setPointSize(
            18
        ) if sys.platform == "darwin" else fontTitle.setPointSize(12)
        fontSubtitle = QtGui.QFont()
        fontSubtitle.setPointSize(
            14
        ) if sys.platform == "darwin" else fontTitle.setPointSize(11)
        fontSmallTitle = QtGui.QFont()
        fontSmallTitle.setPointSize(
            11
        ) if sys.platform == "darwin" else fontSmallTitle.setPointSize(8)
        self.fontDockerfileEditor = QtGui.QFont()
        self.fontDockerfileEditor.setFamily(
            "Helvetica Neue" if sys.platform == "darwin" else "Courier New"
        )
        self.fontDockerfileEditor.setPointSize(
            12
        ) if sys.platform == "darwin" else self.fontDockerfileEditor.setPointSize(10)
        ...
        self.lblIcon.setPixmap(
            QtGui.QPixmap(os.path.join(self.base_dir, "icons/builder_icon.png"))
        )
        ...
        self.glayout_title.addWidget(
            self.lblFormTitle, 0, 1, Qt.AlignLeft | Qt.AlignTop
        )
        self.glayout_title.addWidget(
            self.lblBuidingStep, 0, 2, Qt.AlignRight | Qt.AlignTop
        )
        self.glayout_title.addWidget(
            self.lblDockerVersion, 1, 1, Qt.AlignLeft | Qt.AlignBottom
        )
        self.glayout_title.addWidget(
            self.lblBuiding, 1, 2, Qt.AlignRight | Qt.AlignBottom
        )
    ...
    def retranslateUi(self, Widget):
        ...
        self.lblFormTitle.setText(
            _translate("Widget", "Bioconductor Docker Image Builder")
        )
        self.lblSubtitle.setText(
            _translate("Widget", "Customize your own bioconductor image")
        )
    ...
    def InitializeUI(self):
        ...
        dockerfiles = [
            x for x in fnmatch.filter(os.listdir(self.dockerfile_dir), "*.Dockerfile")
        ]
    ...
    def LoadBiocPakageList(self):
        ...
        self.loadpackage_thread.load_completed.connect(
            self.ThreadEvent_OnLoadBiocPackageCompleted
        )
    ...
    def _update_bioc_package_in_dockerfile(self, previous_package):
        ...
        if matched:
            ...
            if not matched_cmd:
                matched_cmd = self._find_bioclite("WORKDIR", plainText)
            if matched_cmd:
    ...
    def OnSaveDockerfile(self):
        ...
        filename, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "Save Dockerfile", filename, "Docker files (*)"
        )
    ...
    def OnLoadDockerfile(self):
        filename, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, "Open Dockerfile", self.dockerDir, "Docker files (*)"
        )
        if filename:
            itemName = "... " + os.path.join(
    ...
    def OnBuildClicked(self):
        ...
        self.buildimage_thread = DockerThread_BuildImage(
            self.dockerClient, imagename, buildpath, shadow_dockerfile
        )
        self.buildimage_thread.build_process.connect(self.ThreadEvent_OnImageBuilding)
        self.buildimage_thread.build_complete.connect(
            self.ThreadEvent_OnImageBuildComplete
        )
