...
def copyWidgets(
    basePath="/media/data/home/lhhung/bwb/BioDepot-workflow-builder/biodepot/"
):
    dirNames = (
        subprocess.check_output(["ls", "newWidgets"])
        .decode("utf-8")
        .strip()
        .split("\n")
    )
    for dirName in dirNames:
        pylinks = (
            subprocess.check_output(
                ["find", "biodepot/" + dirName, "-type", "l", "-name", "*.py"]
            )
            .decode("utf-8")
            .strip()
            .split("\n")
        ...
        for pylink in pylinks:
            ...
            if widgetPath[0] == "/":
                absWidgetPath = (
                    "/media/data/home/lhhung/bwb/BioDepot-workflow-builder/"
                    + widgetPath
                )
            else:
                absWidgetPath = os.path.abspath(
                    "/media/data/home/lhhung/bwb/BioDepot-workflow-builder/biodepot/{}/{}".format(
                        dirName, widgetPath
                    )
                )
...
def makeLinks(
    ...
    dirNames = (
        subprocess.check_output(["ls", "widgets"]).decode("utf-8").strip().split("\n")
    )
    for dirName in dirNames:
        if dirName[0:4] == "Demo":
            continue
