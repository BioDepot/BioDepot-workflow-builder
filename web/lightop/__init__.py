...
# Flask app
app = Flask(
    __name__, static_folder="static", static_url_path="", instance_relative_config=True
)
...
LoggingConfiguration.set(
    logging.DEBUG if os.getenv("DEBUG") else logging.INFO, "lightop.log", name="Web"
)
...
def exception_to_json(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            result = {"error": {"code": 403, "message": ", ".join(e.args)}}
        except (NotImplementedError, RuntimeError, AttributeError) as e:
            result = {"error": {"code": 500, "message": ", ".join(e.args)}}
