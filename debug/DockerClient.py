...
class DockerClient:
    ...
    def to_host_directory(self, path, source, dest):
        ...
        bprint("mapping path {} using source {} to dest {} volume mapping".format(path,source,dest))
        bprint("mapping cleanpath {} using cleansource {} to cleandest {} volume mapping".format(cleanPath,cleanSource,cleanDestination))
        ...
        if cleanPath == cleanDestination:
            bprint("cleanPath is same as cleanDest so return cleanSource {}".format(cleanSource))
            return cleanSource
        if cleanDestination not in cleanPath:
            bprint("cleanDest {} is not in cleanPath {} so we cannot map it and None is returned")
            return None
