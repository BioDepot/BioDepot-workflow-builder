...
# 1.4 Program
PROG_DIR="/root/Programs/Broad-DGE"
...
while [ "$IDX" -le "${LANES}" ]; do
	...
	echo "/usr/bin/python2.7 ${PROG_DIR}/split_and_align.py" "${SAMPLE_ID}" "${SUBSAMPLE_ID}" "${SEQ_FILE_R1}" "${SEQ_FILE_R2}" "${ALIGN_DIR}/" "${REF_SEQ_FILE} 2> ${TOP_DIR}/pyTimeLogs/pSplitTimeLog$IDX "
 /usr/bin/python2.7 "${PROG_DIR}/split_and_align.py" "${SAMPLE_ID}" "${SUBSAMPLE_ID}" "${SEQ_FILE_R1}" "${SEQ_FILE_R2}" "${ALIGN_DIR}/" "${REF_SEQ_FILE}" 
	# Align individual intermediate FASTQ files.
	for SEQ_FILE in $(find "${ALIGN_DIR}" -maxdepth 1 -name "${SAMPLE_ID}\.${SUBSAMPLE_ID}*\.fastq"); do
...
/usr/bin/python2.7 "${PROG_DIR}/merge_and_count.py" "${SAMPLE_ID}" "${SYM2REF_FILE}" "${ERCC_SEQ_FILE}" "${BARCODE_FILE}" "${ALIGN_DIR}/" "${COUNT_DIR}/" False 
