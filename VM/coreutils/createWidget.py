...
def createWidget(inputJson, outputWidget, widgetName, inputData=None):
    ...
    with open(outputWidget, "w") as f:
        ...
        if "inputs" in data and data["inputs"]:
            ...
            for attr, values in data["inputs"].items():
                if "callback" in values and values["callback"]:
                    inputStr = inputStr + '("{}",{},"{}"),'.format(
                        attr, deClass(str(values["type"])), values["callback"]
                    )
                else:
                    inputStr = inputStr + '("{}",{},"handleInputs{}"),'.format(
                        attr, deClass(str(values["type"])), attr
                    )
            inputStr = inputStr[:-1] + "]"
            f.write("    inputs = {}\n".format(inputStr))
        ...
        if "parameters" in data and data["parameters"]:
            for pname, pvalue in data["parameters"].items():
                if "default" in pvalue and pvalue["default"] is not None:
                    ...
                    if (
                        pvalue["type"] == "int"
                        or pvalue["type"] == "float"
                        or pvalue["type"] == "double"
                        or pvalue["type"] == "bool"
                        or pvalue["type"][-4:] == "dict"
                        or pvalue["type"][-4:] == "Dict"
                        or pvalue["type"][-4:] == "list"
                    ):
        ...
        f.write(
            '        with open(getJsonName(__file__,"{}")) as f:\n'.format(widgetName)
        )
        ...
        if "inputs" in data and data["inputs"]:
            for attr, values in data["inputs"].items():
                ...
                f.write(
                    '            self.handleInputs("{}", value, args[0][0], test=args[0][3])\n'.format(
                        attr
                    )
                )
                f.write("        else:\n")
