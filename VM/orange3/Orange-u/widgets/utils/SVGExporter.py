...
def _generateItemSvg(item, nodes=None, root=None):
    ...
    try:
        ...
        g2 = [
            n for n in g1.childNodes if isinstance(n, xml.Element) and n.tagName == "g"
        ]
    ...
    if not isinstance(item, QtGui.QGraphicsScene):
        ...
        if int(item.flags() & item.ItemClipsChildrenToShape) > 0:
            ...
            try:
                ...
                pathNode = _generateItemSvg(path, root=root)[0].getElementsByTagName(
                    "path"
                )[0]
    ...
    for grp in groups:
        ...
        for ch in grp.childNodes:
            ...
            if ch.tagName == "polyline":
                ...
                ch.setAttribute(
                    "points", " ".join([",".join([str(a) for a in c]) for c in coords])
                )
...
def itemTransform(item, root):
    ...
    if int(item.flags() & item.ItemIgnoresTransformations) > 0:
        ...
        while True:
            ...
            if (
                nextRoot is root
                or int(nextRoot.flags() & nextRoot.ItemIgnoresTransformations) > 0
            ):
                break
