...
class TestParseUri(unittest.TestCase):
    def test_parses_connection_uri(self):
        parameters = parse_uri("sql://user:password@host:7678/database/table")
    ...
    def test_parse_minimal_connection_uri(self):
        parameters = parse_uri("sql://host/database/table")

        self.assertDictContainsSubset(
            dict(host="host", database="database", table="table"), parameters
        )
...
class PostgresTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        Psycopg2Backend.connection_pool = ThreadedConnectionPool(
            1, 1, **connection_params()
        )
    ...
    def sql_table_from_data(self, data, guess_values=True):
        ...
        yield SqlTable(connection_params(), table_name, inspect_values=guess_values)
    ...
    def _create_sql_table(self, data, sql_column_types=None):
        ...
        if sql_column_types is None:
            ...
            sql_column_types = [
                "float" if size == 0 else "varchar(%s)" % size for size in column_size
            ]
        ...
        for row in data:
            ...
            INSERT INTO "%(table_name)s" VALUES
                (%(values)s)
            """ % dict(
                table_name=table_name, values=", ".join(values)
            )
