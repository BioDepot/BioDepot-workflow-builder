...
class TestSimpleTreeLearner(unittest.TestCase):
    def setUp(self):
        ...
        di = [
            Orange.data.domain.DiscreteVariable("d{}".format(i), ["0", "1"])
            for i in range(self.Mi)
        ]
        df = [
            Orange.data.domain.ContinuousVariable("c{}".format(i))
    ...
    def test_SimpleTree_to_string_classification(self):
        domain = Domain(
            [DiscreteVariable(name="d1", values="ef"), ContinuousVariable(name="c1")],
