...
class TestSqlTable(PostgresTest):
    ...
    def test_XY_small(self):
        ...
        sql_table = SqlTable(
            ...
            table_name,
            type_hints=Domain(
                [], DiscreteVariable(name="col2", values=["0", "1", "2"])
            ),
        )
    ...
    def test_joins(self):
        table = SqlTable(
            ...
            type_hints=Domain(
                [DiscreteVariable(name="qualitative petal length", values=["<", ">"])],
                [],
            ),
        )
    ...
    def test_metas_type_hints(self):
        iris = SqlTable(
            self.conn, self.iris, type_hints=Domain([], [], metas=[self.IRIS_VARIABLE])
        )
    ...
    def test_select_all(self):
        iris = SqlTable(
            self.conn, "SELECT * FROM iris", type_hints=Domain([], self.IRIS_VARIABLE)
        )
    ...
    def test_time_time(self):
        table = np.array(
            ["17:39:51", "11:51:48.46", "05:20:21.492149", "21:47:06", "04:47:35.8"]
        ).reshape(-1, 1)
    ...
    def test_time_timestamptz(self):
        table = np.array(
            [
                "2014-07-15 17:39:51.348149+0200",
    ...
    @unittest.skipIf(sql_version < 90200, "Type not supported on this server version.")
    ...
    def test_recovers_connection_after_sql_error(self):
        ...
        try:
            broken_query = "SELECT 1/%s FROM %s" % (
                sql_table.domain.attributes[0].to_sql(),
    ...
    def test_basic_stats(self):
        ...
        self.assertEqual(
            len(domain_stats.stats), len(iris.domain) + len(iris.domain.metas)
        )
    ...
    def test_contingencies(self):
        iris = SqlTable(self.conn, self.iris, inspect_values=True)
        iris.domain = Domain(
            iris.domain[:2] + (EqualWidth()(iris, iris.domain["sepal width"]),),
            iris.domain["iris"],
        )
