...
class TestTreeAdapter(unittest.TestCase):
    ...
    def test_adapter(self):
        ...
        np.testing.assert_almost_equal(
            adapt.get_distribution(self.left), np.array([[1, 42]])
        )
        ...
        self.assertEqual(
            adapt.leaves(self.root), self.left.children + [self.right.children[0]]
        )
        np.testing.assert_almost_equal(
            adapt.get_instances_in_nodes(self.root).X, self.data.X
        )
        np.testing.assert_almost_equal(
