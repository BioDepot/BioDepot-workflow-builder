...
class FeatureConstructorTest(unittest.TestCase):
    def test_construct_variables_discrete(self):
        ...
        expression = (
            "iris_one if iris == 'Iris-setosa' else iris_two "
    ...
    def test_construct_variables_continuous(self):
        ...
        featuremodel = PyListModel(
            [
                ContinuousDescriptor(
                    name=name, expression=expression, number_of_decimals=2
                )
            ]
        )
        data = Table(
        ...
        for i in range(3):
            self.assertEqual(
                data[i * 50, name], pow(data[i * 50, 0] + data[i * 50, 1], 2)
            )
    ...
    def test_construct_numeric_names(self):
        ...
        desc = PyListModel(
            [
                ContinuousDescriptor(
                    name="S", expression="_0_1 + _1", number_of_decimals=3
                )
            ]
        )
...
class TestTools(unittest.TestCase):
    def test_free_vars(self):
        ...
        if sys.version_info >= (3, 5):
            self.assertEqual(
                freevars_("f(*a, *b, k=c, **d, **e)", ["f"]), ["a", "b", "c", "d", "e"]
            )
...
class FeatureFuncTest(unittest.TestCase):
    ...
    def test_call(self):
        ...
        iris = Table("iris")
        f = FeatureFunc(
            "sepal_width + 10", [("sepal_width", iris.domain["sepal width"])]
        )
        r = f(iris)
