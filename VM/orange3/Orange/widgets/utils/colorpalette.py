...
class ColorPaletteDlg(QDialog, gui.OWComponent):
    ...
    def createContinuousPalette(
        ...
        boxCaption,
        passThroughBlack=0,
        initialColor1=Qt.blue,
        initialColor2=Qt.yellow,
    ):
    ...
    def createExtendedContinuousPalette(
        ...
        for i, (color, check) in enumerate(extendedPassThroughColors):
            ...
            _set(
                "passThroughColor" + str(i) + "Checkbox",
                gui.checkBox(
                    box,
                    self,
    ...
    def createDiscretePalette(
        self, paletteName, boxCaption, rgbColors=DefaultRGBColors
    ):
        def _set(keypart, val):
        ...
        for paletteName in self.discPaletteNames:
            if self.__dict__["disc" + paletteName + "EditButt"].isChecked():
                ...
                if dlg.exec_() and colors != dlg.getRgbColors():
                    self.__dict__["disc" + paletteName + "View"].setDiscPalette(
                        dlg.getRgbColors()
                    )
    ...
    def getExtendedContinuousPalette(self, paletteName):
        ...
        if colors:
            colors = [
                self.__dict__["exCont" + paletteName + "color" + str(i)].getColor()
    ...
    def getCurrentState(self):
        l1 = [
            (name, self.qRgbFromQColor(self.__dict__["butt" + name].getColor()))
        ...
        l2 = [
            (
                ...
                (
                    self.qRgbFromQColor(
                        self.__dict__["cont" + name + "Left"].getColor()
                    ),
                    self.qRgbFromQColor(
                        self.__dict__["cont" + name + "Right"].getColor()
                    ),
        ...
        l3 = [
            (name, self.__dict__["disc" + name + "View"].rgbColors)
        ...
        l4 = [
            (
                ...
                (
                    self.qRgbFromQColor(
                        self.__dict__["exCont" + name + "Left"].getColor()
                    ),
                    self.qRgbFromQColor(
                        self.__dict__["exCont" + name + "Right"].getColor()
                    ),
                    ...
                    [
                        (
                            self.qRgbFromQColor(
                                self.__dict__[
                                    "exCont" + name + "color" + str(i)
                                ].getColor()
                            ),
                            self.__dict__[
                                "exCont" + name + "passThroughColor" + str(i)
                            ],
                        )
    ...
    def setCurrentState(self, state):
        ...
        for (name, (l, r, chk)) in contPalettes:
            ...
            self.__dict__["cont" + name + "View"].setContPalette(
                rgbToQColor(l), rgbToQColor(r), chk
            )
        ...
        for name, (l, r, chk, colors) in exContPalettes:
            ...
            for i, (color, check) in enumerate(colors):
                ...
                self.__dict__[
                    "exCont" + name + "passThroughColor" + str(i) + "Checkbox"
                ].setChecked(check)
                self.__dict__["exCont" + name + "color" + str(i)].setColor(
                    rgbToQColor(color)
                )
                if check and chk:
                    colorsList.append(rgbToQColor(color))
            self.__dict__["exCont" + name + "colorCount"] = self.__dict__.get(
                "exCont" + name + "colorCount", len(colors)
            )
        ...
        if self.selectedSchemaIndex == self.schemaCombo.count() - 1:
            ...
            while not ok:
                text, ok = QInputDialog.getText(
                    self, "Name Your Color Settings", message
                )
                if ok:
                    newName = str(text)
                    oldNames = [
                        str(self.schemaCombo.itemText(i)).lower()
    ...
    def createPalette(self, color1, color2, passThroughBlack, colorNumber=250):
        if passThroughBlack:
            palette = [
                qRgb(
                    color1.red() - color1.red() * i * 2.0 / colorNumber,
                    color1.green() - color1.green() * i * 2.0 / colorNumber,
                    color1.blue() - color1.blue() * i * 2.0 / colorNumber,
                )
                for i in range(colorNumber / 2)
            ]
            palette += [
                qRgb(
                    color2.red() * i * 2.0 / colorNumber,
                    color2.green() * i * 2.0 / colorNumber,
                    color2.blue() * i * 2.0 / colorNumber,
                )
            ...
            palette = [
                qRgb(
                    color1.red() + (color2.red() - color1.red()) * i / colorNumber,
                    color1.green()
...
class ColorPalleteListing(QDialog):
    def __init__(self, parent=None, windowTitle="Color Palette List", **kwargs):
        ...
        buttons = QDialogButtonBox(QDialogButtonBox.Cancel, rejected=self.reject)
...
class _ColorButton(QPushButton):
    ...
    def _paletteicon(self, colors, size):
        return QIcon(createDiscPalettePixmap(size.width(), size.height(), colors))


class PaletteEditor(QDialog):
    def __init__(self, rgbColors, parent=None, windowTitle="Palette Editor", **kwargs):
        super().__init__(parent, **kwargs)
        self.setLayout(QVBoxLayout())
        self.layout().setContentsMargins(4, 4, 4, 4)
        ...
        vbox = gui.vBox(hbox)
        buttonUPAttr = gui.button(
            vbox, self, "", callback=self.moveAttrUP, tooltip="Move selected colors up"
        )
        ...
        for ind, (r, g, b) in enumerate(rgbColors):
            item = QListWidgetItem(
                ColorPixmap(QColor(r, g, b), 25), "Color %d" % (ind + 1)
            )
            item.rgbColor = (r, g, b)
    ...
    def moveAttrUP(self):
        if len(self.discListbox.selectedIndexes()) == 0:
            return
        ...
        self.discListbox.item(ind - 1).rgbColor, self.discListbox.item(ind).rgbColor = (
            self.discListbox.item(ind).rgbColor,
            self.discListbox.item(ind - 1).rgbColor,
        )
    ...
    def moveAttrDOWN(self):
        if len(self.discListbox.selectedIndexes()) == 0:
            return
        ind = self.discListbox.selectedIndexes()[0].row()
        if ind == self.discListbox.count() - 1:
            return
        iconI = self.discListbox.item(ind + 1).icon()
        iconII = self.discListbox.item(ind).icon()
        self.discListbox.item(ind + 1).setIcon(iconII)
    ...
    def getRgbColors(self):
        return [
            self.discListbox.item(i).rgbColor for i in range(self.discListbox.count())
        ]


EPS = 7.0 / 3 - 4.0 / 3 - 1  # http://stackoverflow.com/a/25155518/1090455
...
class GradientPaletteGenerator:
    ...
    def getRGB(self, values):
        ...
        results = np.round(
            (1 - p) * self.colors[bin - 1].T + p * self.colors[bin].T
        ).T.astype(int)
        ...
        self.gammaFunc = (
            lambda x, gamma: (
                (
                    ...
                    if x > 0.5
                    else -math.exp(gamma * math.log(-2 * x + 1))
                    if x != 0.5
                    else 0.0
                )
                + 1
    ...
    def set_number_of_colors(self, number_of_colors=0):
        ...
        if isinstance(self.rgb_colors, dict):
            ...
            if number_of_colors not in self.rgb_colors:
                try:
                    number_of_colors = min(
                        [n for n in self.rgb_colors if n >= number_of_colors]
                    )
                except ValueError:
...
class ColorPaletteBW:
    def __init__(self, numberOfColors=-1, brightest=50, darkest=255):
        ...
        if numberOfColors == -1:
            ...
            self.values = [
                int(brightest + (darkest - brightest) * x / float(numberOfColors - 1))
        ...
        for name, (c1, c2, colors) in palettes:
            icon = QIcon(
                createExContPalettePixmap(
                    self.iconSize().width(), self.iconSize().height(), c1, c2, colors
                )
            )
