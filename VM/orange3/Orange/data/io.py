...
def detect_encoding(filename):
    ...
    if isinstance(filename, str) and not filename.endswith(Compression.all):
        try:
            with subprocess.Popen(
                ("file", "--brief", "--mime-encoding", filename), stdout=subprocess.PIPE
            ) as process:
                process.wait()
    ...
    def _from_file(f):
        ...
        detector.close()
        return (
            detector.result.get("encoding")
            if detector.result.get("confidence", 0) >= 0.85
            else "utf-8"
        )
...
def sanitize_variable(
    ...
    def get_number_of_decimals(values):
        len_ = len
        ndecimals = max(
            (len_(value) - value.find(".") for value in values if "." in value),
            default=1,
        )
        return ndecimals - 1
    ...
    if isinstance(var, DiscreteVariable):
        ...
        # here)
        mapping = defaultdict(
            lambda: np.nan, {val: i for i, val in enumerate(var.values)}
        )
        mapping[""] = np.nan
...
class Flags:
    ...
    def join(iterable, *args):
        return Flags.DELIMITER.join(
            i.strip().replace(Flags.DELIMITER, "\\" + Flags.DELIMITER)
...
class FileFormat(metaclass=FileFormatMeta):
            ...
            # Unpickling throws different exceptions, not just UnpickleError
                ...
                with open(filename + ".metadata", encoding="utf-8") as f:
                    table.attributes = OrderedDict(
                        (k.strip(), v.strip())
                        for k, v in (line.split(":", 1) for line in f.readlines())
                    )
    ...
    def data_table(cls, data, headers=None):
        ...
        if len(headers) == 3:
            ...
            if len(headers) == 1:
                ...
                _flags, names = zip(
                    *[
                        i.split(HEADER1_FLAG_SEP, 1)
                        if HEADER1_FLAG_SEP in i
                        else ("", i)
                        for i in headers[0]
                    ]
                )
        ...
        data = [_equal_length([s.strip() for s in row]) for row in data if any(row)]
        ...
        for col in range(rowlen):
            ...
            if type_flag in StringVariable.TYPE_HEADERS:
                ...
                try:
                    np.copyto(values, orig_values, casting="unsafe", where=~namask)
                    values[namask] = np.nan
                except ValueError:
                    for row, num in enumerate(orig_values):
                        if not isnastr(num):
                            try:
                                ...
                                break
                    raise ValueError(
                        "Non-continuous value in (1-based) "
                        "line {}, column {}".format(row + len(headers) + 1, col + 1)
                    )

            elif type_flag in TimeVariable.TYPE_HEADERS:
                coltype = TimeVariable
                values = np.where(namask, "", orig_values)
            elif type_flag in DiscreteVariable.TYPE_HEADERS or _RE_DISCRETE_LIST.match(
                type_flag
            ):
                coltype = DiscreteVariable
    ...
    def header_names(data):
        return ["weights"] * data.has_weights() + [
            v.name
            for v in chain(
                data.domain.attributes, data.domain.class_vars, data.domain.metas
            )
        ]
        ...
        return list(
            chain(
                ...
                (
                    Flags.join(
                        [flag],
                        *("{}={}".format(*a) for a in sorted(var.attributes.items()))
                    )
                    for flag, var in chain(
    ...
    def read(self):
        for encoding in (
            ...
            if errors != "ignore":
                error = ""
            with self.open(
                self.filename, mode="rt", newline="", encoding=encoding, errors=errors
            ) as file:
                ...
                try:
                    ...
                    if error and isinstance(error, UnicodeDecodeError):
                        ...
                        warning = (
                            "Skipped invalid byte(s) in position " "{}{}"
                        ).format(pos, ("-" + str(endpos)) if (endpos - pos) > 1 else "")
                    ...
                    continue
        raise ValueError(
            "Cannot parse dataset {}: {}".format(self.filename, error)
        ) from error
...
class BasketReader(FileFormat):
    ...
    def read(self):
        def constr_vars(inds):
            if inds:
                return [
                    ContinuousVariable(x.decode("utf-8"))
                    for _, x in sorted((ind, name) for name, ind in inds.items())
                ]

        X, Y, metas, attr_indices, class_indices, meta_indices = _io.sparse_read_float(
            self.filename.encode(sys.getdefaultencoding())
        )
...
class UrlReader(FileFormat):
    ...
    @classmethod
    def _trim(cls, url):
        URL_TRIMMERS = (cls._trim_googlesheet, cls._trim_dropbox)
    ...
    def _trim_googlesheet(url):
        ...
        try:
            ...
            raise ValueError
        url = "https://docs.google.com/spreadsheets/d/{}/export?format=tsv".format(
            workbook
        )
        if sheet:
            url += "&gid=" + sheet
    ...
    def _suggest_filename(self, content_disposition):
        ...
        matches = re.findall(
            r"filename\*?=(?:\"|.{0,10}?'[^']*')([^\"]+)", content_disposition or ""
        )
