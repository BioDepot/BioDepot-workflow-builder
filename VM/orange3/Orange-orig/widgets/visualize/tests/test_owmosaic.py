...
class TestOWMosaicDisplay(WidgetTest, WidgetOutputsTestMixin):
    ...
    def test_continuous_metas(self):
        ...
        data = Table(
            domain, np.arange(6).reshape(6, 1), metas=np.arange(6).reshape(6, 1)
        )
    ...
    def test_string_meta(self):
        ...
        data = Table(
            domain, np.empty((6, 0)), metas=np.array(["meta"] * 6).reshape(6, 1)
        )
        self.send_signal(self.widget.Inputs.data, data)
    ...
    def test_missing_values(self):
        ...
        data = Table(
            Domain([DiscreteVariable("c1", [])]), np.array([np.nan] * 6)[:, None]
        )
        self.send_signal(self.widget.Inputs.data, data)
...
class MosaicVizRankTests(WidgetTest):
    ...
    def test_iteration(self):
        ...
        vizrank.max_attrs = 4
        self.assertEqual(
            [state.copy() for state in vizrank.iterate_states(None)],
            [
                [0],
                [1],
                [2],
        ...
        self.assertEqual(
            [state.copy() for state in vizrank.iterate_states([0, 3])],
            [
                [0, 3],
                [1, 3],
                [2, 3],
    ...
    def test_nan_column(self):
        ...
        table = Table(
            ...
            np.array([[0, np.NaN, 0], [0, np.NaN, 0], [0, np.NaN, 0]]),
        )
    ...
    def test_color_combo(self):
        ...
        GH-2036
        """
        RESULTS = [[0, 2, 6], [0, 3, 10], [0, 4, 11], [1, 2, 6], [1, 3, 7], [1, 4, 7]]
        table = Table("titanic")
        self.send_signal(self.widget.Inputs.data, table)
