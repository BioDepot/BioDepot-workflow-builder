...
class OWPurgeDomain(widget.OWWidget):
    name = "Purge Domain"
    description = (
        "Remove redundant values and features from the dataset. " "Sort values."
    )
    ...
    def __init__(self):
        ...
        for not_first, (value, label) in enumerate(self.feature_options):
            ...
            gui.checkBox(boxAt, self, value, label, callback=self.optionsChanged)
        ...
        gui.auto_commit(
            self.buttonsArea, self, "autoSend", "Apply", orientation=Qt.Horizontal
        )
    ...
    def commit(self):
        ...
        attr_res, class_res, meta_res = (
            remover.attr_results,
            remover.class_results,
            remover.meta_results,
        )
    ...
    def send_report(self):
        def list_opts(opts):
            return (
                "; ".join(
                    label.lower() for value, label in opts if getattr(self, value)
                )
                or "no changes"
            )
