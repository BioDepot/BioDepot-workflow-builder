...
class OWDataInfo(widget.OWWidget):
    ...
    def __init__(self):
        ...
        for box in (
            ...
            bo = gui.vBox(
                self.controlArea, box, addSpace=False and box != "Meta Attributes"
            )
    ...
    def data(self, data):
        ...
        self.data_set_size = (
            pack_table(
                (("Rows", "~{}".format(data.approx_len())), ("Variables", len(domain)))
            )
            + sparses
        )
        ...
        def update_size():
            self.data_set_size = (
                pack_table((("Rows", len(data)), ("Variables", len(domain)))) + sparses
            )
        ...
        if class_var:
            if class_var.is_continuous:
                ...
                self.targets = "Discrete outcome with %i values" % len(class_var.values)
            ...
            if not cont_class:
                self.targets = "Multitarget data,\n%i categorical targets" % n_or_none(
                    disc_class
                )
            elif not disc_class:
                self.targets = "Multitarget data,\n%i numeric targets" % n_or_none(
                    cont_class
                )
            else:
