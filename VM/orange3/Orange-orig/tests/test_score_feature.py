...
class FeatureScoringTest(unittest.TestCase):
    ...
    def test_info_gain(self):
        ...
        np.testing.assert_almost_equal(
            [scorer(self.zoo, a) for a in range(5)], correct, decimal=5
        )
    ...
    def test_chi2(self):
        ...
        domain = Domain(
            domain.attributes,
            DiscreteVariable("c", values=[str(v) for v in np.unique(y)]),
        )
    ...
    def test_relieff(self):
        ...
        found = [
            self.monk.domain[attr].name for attr in reversed(weights.argsort()[-3:])
        ]
        reference = ["a", "b", "e"]
    ...
    def test_fcbf(self):
        ...
        found = [
            self.zoo.domain[attr].name for attr in reversed(weights.argsort()[-5:])
        ]
        ...
        data = Table(
            Domain(
                [ContinuousVariable("1"), ContinuousVariable("2")],
