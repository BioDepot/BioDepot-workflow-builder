    ...
    def fit(self, X, Y=None):
        ...
        try:
            if self._compute_silhouette and 2 <= proj.n_clusters < X.shape[0]:
                if len(X) <= 5000:
                    proj.silhouette_samples = silhouette_samples(X, proj.labels_)
                    proj.silhouette = np.mean(proj.silhouette_samples)
                else:
                    proj.silhouette_samples = None
                    proj.silhouette = silhouette_score(
                        X, proj.labels_, sample_size=5000
                    )
        ...
        proj.inter_cluster = np.mean(
            cluster_dist[np.triu_indices_from(cluster_dist, 1)]
        )
...
class KMeansModel(Projection):
    ...
    def __call__(self, data):
        if isinstance(data, Table):
            ...
            c = DiscreteVariable(
                name="Cluster id", values=[str(i) for i in range(self.k)]
            )
            domain = Domain([c])
            return Table(
                domain, self.proj.predict(data.X).astype(int).reshape((len(data), 1))
            )
