...
class CommonTests:
    ...
    def test_no_data(self):
        ...
        np.testing.assert_almost_equal(
            self.Distance(Table(self.domain)), np.zeros((0, 0))
        )
        np.testing.assert_almost_equal(
            self.Distance(self.data, Table(self.domain)), np.zeros((n, 0))
        )
        np.testing.assert_almost_equal(
            self.Distance(Table(self.domain), self.data), np.zeros((0, n))
        )
...
class CommonNormalizedTests(CommonFittedTests):
    ...
    def test_zero_variance(self):
        ...
        def is_same(d1, d2, fit1=None, fit2=None):
            ...
            if fit2 is None:
                fit2 = d2
            assert_almost_equal(normalized.fit(fit1)(d1), normalized.fit(fit2)(d2))
            assert_almost_equal(
                nonnormalized.fit(fit1)(d1), nonnormalized.fit(fit2)(d2)
            )
        ...
        self.assertRaises(ValueError, self.Distance, data_const, axis=0, normalize=True)
        self.assertRaises(ValueError, self.Distance, data_nan, axis=0, normalize=True)
        self.assertRaises(ValueError, self.Distance, data_nan_1, axis=0, normalize=True)
    ...
    def test_mixed_cols(self):
        ...
        self.assertRaises(ValueError, self.Distance(axis=0).fit, self.mixed_data)
...
class FittedDistanceTest(unittest.TestCase):
    ...
    def setUp(self):
        self.cont_data = Table.from_list(
            self.cont_domain, [[1, 3, 2], [-1, 5, 0], [1, 1, 1], [7, 2, 3]]
        )

        self.cont_data2 = Table.from_list(self.cont_domain, [[2, 1, 3], [1, 2, 2]])

        self.disc_data = Table.from_list(
            self.disc_domain, [[0, 0, 0], [0, 1, 1], [1, 3, 1]]
        )

        self.disc_data4 = Table.from_list(
            self.disc_domain, [[0, 0, 0], [0, 1, 1], [0, 1, 1], [1, 3, 1]]
        )
...
class EuclideanDistanceTest(FittedDistanceTest, CommonNormalizedTests):
    ...
    def test_euclidean_disc(self):
        ...
        assert_almost_equal(
            ...
            [[1 / 3, 2 / 3, 1, 1], [2 / 3, 2 / 3, 1, 2 / 3], [2 / 3, 1 / 3, 1, 1]],
        )
        assert_almost_equal(model.dist_missing2_disc, [1 - 5 / 9, 1 - 3 / 9, 1 - 5 / 9])

        dist = model(data)
        assert_almost_equal(dist, np.sqrt(np.array([[0, 2, 3], [2, 0, 2], [3, 2, 0]])))

        data.X[1, 0] = np.nan
        model = distance.Euclidean().fit(data)
        assert_almost_equal(
            model.dist_missing_disc,
            [[1 / 2, 1 / 2, 1, 1], [2 / 3, 2 / 3, 1, 2 / 3], [2 / 3, 1 / 3, 1, 1]],
        )
        assert_almost_equal(model.dist_missing2_disc, [1 - 2 / 4, 1 - 3 / 9, 1 - 5 / 9])
        ...
        assert_almost_equal(model.dist_missing2_disc, [1 - 1, 1 - 3 / 9, 1 - 5 / 9])

        dist = model(data)
        assert_almost_equal(dist, np.sqrt(np.array([[0, 2, 2], [2, 0, 1], [2, 1, 0]])))
        ...
        assert_almost_equal(
            model.dist_missing_disc,
            [[1 / 2, 1 / 2, 1, 1], [3 / 4, 2 / 4, 1, 3 / 4], [3 / 4, 1 / 4, 1, 1]],
        )
        assert_almost_equal(
            model.dist_missing2_disc, [1 - 2 / 4, 1 - 6 / 16, 1 - 10 / 16]
        )
    ...
    def test_euclidean_cont(self):
        ...
        assert_almost_equal(
            ...
            np.sqrt(
                np.array(
                    [[0, 12, 5, 38], [12, 0, 21, 82], [5, 21, 0, 41], [38, 82, 41, 0]]
                )
            ),
        )
    ...
    def test_euclidean_cols_normalized(self):
        ...
        assert_almost_equal(
            dist,
            [[0, 2, 0.649839392], [2, 0, 1.704275472], [0.649839392, 1.704275472, 0]],
        )
        ...
        assert_almost_equal(
            dist,
            [[0, 2, 1.450046001], [2, 0, 1.704275472], [1.450046001, 1.704275472, 0]],
        )
    ...
    def test_two_tables(self):
        ...
        dist = distance.Euclidean(self.cont_data, self.cont_data2, normalize=True)
        assert_almost_equal(
            dist,
            [
...
class ManhattanDistanceTest(FittedDistanceTest, CommonNormalizedTests):
    ...
    def setUp(self):
        super().setUp()
        self.cont_data = Table.from_list(
            self.cont_domain, [[1, 3, 2], [-1, 6, 0], [2, 7, 1], [7, 1, 3]]
        )

    def test_manhattan_no_data(self):
        np.testing.assert_almost_equal(
            distance.Manhattan(Table(self.domain)), np.zeros((0, 0))
        )
        np.testing.assert_almost_equal(
            distance.Manhattan(self.mixed_data, Table(self.domain)), np.zeros((3, 0))
        )
        np.testing.assert_almost_equal(
            distance.Manhattan(Table(self.domain), self.mixed_data), np.zeros((0, 3))
        )
        self.assertRaises(
            ValueError,
            distance.Manhattan,
            Table(self.cont_domain),
            axis=0,
            normalize=True,
        )
    ...
    def test_manhattan_cont(self):
        ...
        assert_almost_equal(
            dist, [[0, 7, 6, 9], [7, 0, 5, 16], [6, 5, 0, 13], [9, 16, 13, 0]]
        )

        data.X[1, 0] = np.nan
        dist = distance.Manhattan(data, axis=1, normalize=False)
        assert_almost_equal(
            dist, [[0, 7, 6, 9], [7, 0, 3, 14], [6, 3, 0, 13], [9, 14, 13, 0]]
        )

        data.X[0, 0] = np.nan
        dist = distance.Manhattan(data, axis=1, normalize=False)
        assert_almost_equal(
            dist, [[0, 10, 10, 8], [10, 0, 7, 13], [10, 7, 0, 13], [8, 13, 13, 0]]
        )
    ...
    def test_manhattan_cont_normalized(self):
        ...
        assert_almost_equal(
            dist,
            [[0, 2.75, 2, 4], [2.75, 0, 1.25, 5.75], [2, 1.25, 0, 5], [4, 5.75, 5, 0]],
        )
    ...
    def test_manhattan_cols(self):
        ...
        assert_almost_equal(dist, [[0, 20, 7], [20, 0, 15], [7, 15, 0]])

        data.X[1, 1] = np.nan
        dist = distance.Manhattan(data, axis=0, normalize=False)
        assert_almost_equal(dist, [[0, 19, 7], [19, 0, 14], [7, 14, 0]])

        data.X[1, 0] = np.nan
        dist = distance.Manhattan(data, axis=0, normalize=False)
        assert_almost_equal(dist, [[0, 17, 9], [17, 0, 14], [9, 14, 0]])
    ...
    def test_manhattan_cols_normalized(self):
        ...
        assert_almost_equal(
            dist, [[0, 4.5833333, 2], [4.5833333, 0, 4.25], [2, 4.25, 0]]
        )

        data.X[1, 1] = np.nan
        dist = distance.Manhattan(data, axis=0, normalize=True)
        assert_almost_equal(dist, [[0, 4.6666667, 2], [4.6666667, 0, 4], [2, 4, 0]])

        data.X[1, 0] = np.nan
        dist = distance.Manhattan(data, axis=0, normalize=True)
        assert_almost_equal(dist, [[0, 5.5, 4], [5.5, 0, 4], [4, 4, 0]])
    ...
    def test_two_tables(self):
        ...
        dist = distance.Manhattan(self.cont_data, self.cont_data2, normalize=True)
        assert_almost_equal(
            dist,
            [[1.3333333, 0.25], [3.75, 2.6666667], [2.5, 2.0833333], [1.6666667, 2.75]],
        )
    ...
    def test_manhattan_mixed_cols(self):
        self.assertRaises(ValueError, distance.Manhattan, self.mixed_data, axis=0)
        self.assertRaises(ValueError, distance.Manhattan(axis=0).fit, self.mixed_data)
...
class CosineDistanceTest(FittedDistanceTest, CommonFittedTests):
    ...
    def test_cosine_disc(self):
        assert_almost_equal = np.testing.assert_almost_equal
        data = self.disc_data
        data.X = np.array([[1, 0, 0], [0, 1, 1], [1, 3, 0]], dtype=float)
        ...
        assert_almost_equal(
            dist,
            1 - np.array([[1, 0, 1 / sqrt(2)], [0, 1, 0.5], [1 / sqrt(2), 0.5, 1]]),
        )
        ...
        data.X = np.array([[1, 0, 0], [0, np.nan, 1], [1, np.nan, 1], [1, 3, 1]])
        model = distance.Cosine().fit(data)
        dist = model(data)
    ...
    def test_cosine_mixed(self):
        ...
        data = self.mixed_data
        data.X = np.array(
            [[1, 3, 2, 1, 0, 0], [-1, 5, 0, 0, 1, 1], [1, 1, 1, 1, 3, 0]], dtype=float
        )
...
class JaccardDistanceTest(unittest.TestCase, CommonFittedTests):
    ...
    def setUp(self):
        ...
        self.data = Table(self.domain, [[0, 1, 1], [1, 1, 1], [1, 0, 1], [1, 0, 0]])
    ...
    def test_jaccard_cols(self):
        ...
        assert_almost_equal(
            model(self.data),
            1 - np.array([[1, 1 / 4, 1 / 2], [1 / 4, 1, 2 / 3], [1 / 2, 2 / 3, 1]]),
        )

        self.data.X = np.array(
            [[0, 1, 1], [np.nan, np.nan, 1], [np.nan, 0, 1], [1, 1, 0]]
        )
        model = distance.Jaccard(axis=0).fit(self.data)
        assert_almost_equal(model.ps, [0.5, 2 / 3, 0.75])
        assert_almost_equal(
            model(self.data),
            1 - np.array([[1, 0.4, 0.25], [0.4, 1, 5 / 12], [0.25, 5 / 12, 1]]),
        )
