...
class WebSocketRequestHandler(SimpleHTTPRequestHandler):
    ...
    * handler_id: A sequence number for this connection, appended to record filename
    ...
    def unmask(buf, hlen, plen):
        ...
        if numpy:
            ...
            if plen >= 4:
                ...
                data = numpy.frombuffer(buf, dtype, offset=pstart, count=int(plen / 4))
            ...
            if plen % 4:
                ...
                if sys.byteorder == "big":
                    dtype = dtype.newbyteorder(">")
                mask = numpy.frombuffer(buf, dtype, offset=hlen, count=(plen % 4))
                data = numpy.frombuffer(
                    buf, dtype, offset=pend - (plen % 4), count=(plen % 4)
                )
    ...
    def decode_hybi(buf, base64=False, logger=None):
        ...
        if f["masked"]:
            # unmask payload
            f["payload"] = WebSocketRequestHandler.unmask(buf, f["hlen"], f["length"])
        else:
        ...
        if base64 and f["opcode"] in [1, 2]:
            try:
                ...
                logger.exception("Exception while b64decoding buffer: %s" % (repr(buf)))
                raise
            ...
            for buf in bufs:
                ...
                if self.rec:
                    self.rec.write(
                        "%s,\n"
                        % repr(
                            "{%s{" % tdelta + encbuf[lenhead : len(encbuf) - lentail]
                        )
                    )
    ...
    def recv_frames(self):
        ...
        while buf:
            frame = self.decode_hybi(buf, base64=self.base64, logger=self.logger)
            ...
            if self.rec:
                ...
                if frame["masked"]:
                    recbuf = WebSocketRequestHandler.unmask(
                        buf, frame["hlen"], frame["length"]
                    )
                else:
                    recbuf = buf[frame["hlen"] : frame["hlen"] + frame["length"]]
    ...
    def do_websocket_handshake(self):
        ...
        if ver:
            ...
            self.send_error(
                400,
                "Missing Sec-WebSocket-Version header. Hixie protocols not supported.",
            )
    ...
    def handle_websocket(self):
        ...
        if (
            ...
            self.log_message(
                "%s: Version %s, base64: '%s'", client_addr, self.version, self.base64
            )
            if self.path != "/":
...
class WebSocketServer(object):
    ...
    def get_logger():
        return logging.getLogger(
            "%s.%s" % (WebSocketServer.log_prefix, WebSocketServer.__class__.__name__)
        )
        ...
        if handshake == "":
            ...
            try:
                retsock = ssl.wrap_socket(
                    sock, server_side=True, certfile=self.cert, keyfile=self.key
                )
            except ssl.SSLError:
                _, x, _ = sys.exc_info()
    ...
    def multiprocessing_SIGCHLD(self, sig, stack):
        self.vmsg(
            "Reaping zombies, active child count is %s",
