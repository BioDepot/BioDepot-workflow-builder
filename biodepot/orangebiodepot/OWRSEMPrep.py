...
class OWRSEMPrep(widget.OWWidget):
    name = "RSEM Preparation"
    description = "Step1 of RSEM Analysis: Reference sequence preparation step. It executes the rsem-prepare-reference command"
    category = "RNASeq"
    icon = "icons/rsemprep.svg"
    priority = 10

    image_name = "genomicpariscentre/rsem"
    image_version = "latest"

    inputs = [(".gtf and .fa files", str, "set_aligns")]
    outputs = [("Results", str)]
    ...
    host_results_dir = os.path.expanduser('~') + '/BioDepot/RSEM_prep/Results'
...
class RunAnalysisThread(QThread):
    ...
    commands = [
    "rsem-prepare-reference --gtf /usr/local/RSEM/input/Mus_musculus.GRCm38.82.chr.gtf --bowtie  /usr/local/RSEM/input/Mus_musculus.GRCm38.dna.toplevel.fa /usr/local/RSEM/ref/mouse_ref"
