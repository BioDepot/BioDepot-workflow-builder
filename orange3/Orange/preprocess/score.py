...
class Scorer(_RefuseDataInConstructor, Reprable):
    ...
    def __call__(self, data, feature=None):
        if not data.domain.class_var:
            raise ValueError(
                "{} requires data with a target variable.".format(self.friendly_name)
            )
        if not isinstance(data.domain.class_var, self.class_type):
            raise ValueError(
                "{} requires a {} target variable.".format(
                    self.friendly_name, self._friendly_vartype_name(self.class_type)
                )
            )
        ...
        for var in data.domain.attributes:
            if not isinstance(var, self.feature_type):
                raise ValueError(
                    "{} cannot score {} variables.".format(
                        self.friendly_name, self._friendly_vartype_name(type(var))
                    )
                )
...
class SklScorer(Scorer, metaclass=WrapperMeta):
    ...
    preprocessors = Scorer.preprocessors + [Impute()]
...
class Chi2(SklScorer):
    ...
    preprocessors = SklScorer.preprocessors + [Discretize(remove_const=False)]
...
class LearnerScorer(Scorer):
    ...
    def score_data(self, data, feature=None):
        def average_scores(scores):
            ...
            return [
                sum(scores_grouped[attr]) / len(scores_grouped[attr])
        ...
        model_domain = (
            self.get_learner(data).domain if isinstance(self, Fitter) else self.domain
        )

        if data.domain != model_domain:
            scores = np.array([average_scores(row) for row in scores])

        return scores[:, data.domain.attributes.index(feature)] if feature else scores
...
class ClassificationScorer(Scorer):
    ...
    class_type = DiscreteVariable
    supports_sparse_data = True
    preprocessors = Scorer.preprocessors + [Discretize(remove_const=False)]
    ...
    def score_data(self, data, feature):
        ...
        def score_from_contingency(f):
            ...
            return self.from_contingency(
                cont, 1.0 - np.sum(cont.unknowns) / instances_with_class
            )
...
def _gini(D):
    ...
    return np.sum((1 - np.sum(P ** 2, axis=0)) * np.sum(D, axis=0) / np.sum(D))
        ...
        while True:
            ...
            while True:
                try:
                    ...
                    break
                if (
                    _symmetrical_uncertainty(data, attributes[Fp], attributes[Fq])
                    >= SUqc
                ):
                    del S[-q]
...
class ReliefF(Scorer):
    ...
    def score_data(self, data, feature):
        ...
        if not data.domain.class_var.is_discrete:
            raise ValueError(
                "ReliefF supports classification; use RReliefF " "for regression"
            )
...
class RReliefF(Scorer):
    ...
    def score_data(self, data, feature):
        ...
        if not data.domain.class_var.is_continuous:
            raise ValueError(
                "RReliefF supports regression; use ReliefF " "for classification"
            )
        if isinstance(self.random_state, np.random.RandomState):
            rstate = self.random_state
        else:
...
if __name__ == "__main__":
    ...
    X[np.random.random(X.shape) > 0.95] = np.nan
    y_cls = np.zeros(X.shape[0])
    y_cls[(X[:, 0] > 0.5) ^ (X[:, 1] > 0.6)] = 1
