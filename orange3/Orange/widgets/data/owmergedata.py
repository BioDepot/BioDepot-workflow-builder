...
class OWMergeData(widget.OWWidget):
    ...
    def __init__(self):
        ...
        self.infoBoxData = gui.label(box, self, self.dataInfoText(None), box="Data")
        ...
        radio_width = QApplication.style().pixelMetric(
            QStyle.PM_ExclusiveIndicatorWidth
        )

        def add_option(label, pre_label, between_label, merge_type, model, extra_model):
    ...
    def _add_instanceid_to_models(self):
        needs_id = (
            ...
            and self.extra_data is not None
            and len(np.intersect1d(self.data.ids, self.extra_data.ids))
        )
        for model in (self.model_unique_with_id, self.extra_model_unique_with_id):
    ...
    def _find_best_match(self):
        ...
        def best_match(model, extra_model):
            ...
            for m_a, m_b in product(str_metas, extra_str_metas):
                n_inter = len(
                    np.intersect1d(
                        self.data[:, m_a].metas, self.extra_data[:, m_b].metas
                    )
                )
        ...
        def set_attrs(attr_name, attr_extra_name, attr, extra_attr):
            if (
                getattr(self, attr_name) == INDEX
                and getattr(self, attr_extra_name) == INDEX
            ):
    ...
    def commit(self):
        ...
        if (
            self.data is None
            or len(self.data) == 0
            or self.extra_data is None
            or len(self.extra_data) == 0
        ):
            merged_data = None
        else:
            ...
            if merged_data:
                ...
                var_names = [
                    var.name
                    for var in chain(merged_domain.variables, merged_domain.metas)
                ]
    ...
    def send_report(self):
        # pylint: disable=invalid-sequence-index
        attr = (self.attr_augment_data, self.attr_merge_data, self.attr_combine_data)
        extra_attr = (
            self.attr_augment_extra,
            self.attr_merge_extra,
    ...
    def merge(self):
        ...
        as_string = not (
            isinstance(var_data, ContinuousVariable)
    ...
    def _compute_reduced_extra_data(self, var_extra_data):
        ...
        return self.extra_data[:, [var for var in extra_vars if var not in all_vars]]
    ...
    def _values(data, var, as_string):
        ...
        if not as_string:
            return col
        if var.is_primitive():
            return (var.str_val(val) if not np.isnan(val) else np.nan for val in col)
        else:
    ...
    def _get_keymap(cls, data, var, as_string):
        ...
        """
        return ((val, i) for i, val in enumerate(cls._values(data, var, as_string)))
    ...
    def _augment_indices(self, var_data, extra_map, as_string):
        ...
        keys = (
            extra_map.get(val, -1) for val in self._values(data, var_data, as_string)
        )
        return np.vstack(
            (
    ...
    def _combine_indices(self, var_data, extra_map, as_string):
        ...
        if np.nan in key_map:
            del key_map[np.nan]
        keys = np.fromiter(
            (j for key, j in to_add if key not in key_map), dtype=np.int64
        )
        right_indices = np.vstack((np.full(len(keys), -1, np.int64), keys))
        return np.hstack(
            (self._augment_indices(var_data, extra_map, as_string), right_indices)
        )
    ...
    def _join_array_by_indices(left, right, indices, string_cols=None):
        ...
        res = hstack(
            (prepare(left, indices[0], str_left), prepare(right, indices[1], str_right))
        )
        return res
