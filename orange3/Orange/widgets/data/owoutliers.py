...
class OWOutliers(widget.OWWidget):
    ...
    def __init__(self):
        ...
        gui.appendRadioButton(detection, "One class SVM with non-linear kernel (RBF)")
        ...
        gui.button(self.buttonsArea, self, "Detect Outliers", callback=self.commit)
    ...
    def _get_outliers(self):
        try:
            ...
            self.in_out_info_label.setText(
                "{} inliers, {} outliers".format(len(inliers), len(outliers))
            )
    ...
    def detect_outliers(self):
        if self.outlier_method == self.OneClassSVM:
            ...
            learner = EllipticEnvelopeLearner(
                ...
                if self.empirical_covariance
                else None,
                contamination=self.cont / 100.0,
            )
