...
class TestDomainContextHandler(TestCase):
    def setUp(self):
        self.domain = Domain(
            ...
            metas=[ContinuousVariable("c2"), DiscreteVariable("d4", values="jkl")],
        )
        self.args = (
            self.domain,
            {"c1": Continuous, "d1": Discrete, "d2": Discrete, "d3": Discrete},
    ...
    def test_encode_domain_with_match_class(self):
        ...
        self.assertEqual(
            encoded_attributes,
            {"c1": Continuous, "d1": Discrete, "d2": Discrete, "d3": list("ghi")},
        )
        self.assertEqual(encoded_metas, {"c2": Continuous, "d4": Discrete})
    ...
    def test_encode_domain_with_match_all(self):
        ...
        self.assertEqual(
            encoded_attributes,
            {"c1": Continuous, "d1": list("abc"), "d2": list("def"), "d3": list("ghi")},
        )
    ...
    def test_match_returns_1_if_everything_matches(self):
        ...
        context = Mock(
            values=dict(with_metas=("d1", Discrete), required=("d1", Discrete))
        )
        self.assertEqual(1.0, self.handler.match(context, *self.args))

        # Metas in values
        context = Mock(
            values=dict(with_metas=("d4", Discrete), required=("d1", Discrete))
        )
        self.assertEqual(1.0, self.handler.match(context, *self.args))
    ...
    def test_match_returns_zero_on_incompatible_context(self):
        ...
        context = Mock(
            values=dict(
                with_metas=("d1", Discrete), if_selected=[("u", Discrete)], selected=[0]
            )
        )
        self.assertEqual(0, self.handler.match(context, *self.args))
        ...
        self.assertAlmostEqual(0.667, self.handler.match(context, *self.args), places=2)
    ...
    def test_clone_context(self):
        ...
        self.assertEqual(
            [("d1", Discrete), ("c1", Continuous)], new_values["with_metas"]
        )
    ...
    def test_open_context(self):
        ...
        context = self.create_context(
            self.domain,
            dict(text=("u", -2), with_metas=[("d1", Discrete), ("d2", Discrete)]),
        )
    ...
    def test_open_context_with_imperfect_match(self):
        ...
        self.assertEqual(widget.if_selected, [("c1", Continuous), ("d1", Discrete)])
    ...
    def test_decode_setting(self):
        ...
        val = self.handler.decode_setting(
            setting, (var.name, 100 + vartype(var)), self.domain
        )
        self.assertIs(val, var)

        all_metas_domain = Domain([], metas=[var])
        val = self.handler.decode_setting(
            setting, (var.name, 100 + vartype(var)), all_metas_domain
        )
        self.assertIs(val, var)
...
class SimpleWidget:
    ...
    if_selected = ContextSetting(
        [], required=ContextSetting.IF_SELECTED, selected="selected"
    )
    selected = ""
