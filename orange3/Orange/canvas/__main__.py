...
from AnyQt.QtCore import Qt, QDir, QUrl, QSettings, QThread, pyqtSignal, QT_VERSION
...
from Orange.canvas.application.errorreporting import ErrorReporting, handle_exception
    ...
    if check_updates and time.time() - last_check_time > ONE_DAY:
        ...
        def compare_versions(latest):
            ...
            question.finished.connect(
                lambda: question.clickedButton() == ok
                and QDesktopServices.openUrl(QUrl("https://orange.biolab.si/download/"))
            )
            question.show()
...
def main(argv=None):
    ...
    parser.add_option(
        "-l", "--log-level", help="Logging level (0, 1, 2, 3, 4)", type="int", default=1
    )
    ...
    parser.add_option(
        "--qt", help="Additional arguments for QApplication", type="str", default=None
    )
    ...
    clear_settings_flag = os.path.join(config.widget_settings_dir(), "DELETE_ON_START")
    ...
    try:
        with patch(
            "sys.excepthook",
            ExceptHook(
                stream=stderr, canvas=canvas_window, handledException=handle_exception
            ),
