...
def example_workflows():
    ...
    for ep in workflow_entry_points():
        ...
        try:
            ...
            log.warning("Could not load workflows from %r (%r)", ep.dist, ex)
            continue
        except ImportError:
            log.error("Could not load workflows from %r", ep.dist, exc_info=True)
            continue
        except Exception:
            log.error("Could not load workflows from %r", ep.dist, exc_info=True)
            continue
        ...
        if isinstance(workflows, types.ModuleType):
            ...
            workflows = [ExampleWorkflow(wf, package, ep.dist) for wf in workflows]
        elif isinstance(workflows, (types.FunctionType, types.MethodType)):
            try:
                workflows = example_workflows()
            except Exception as ex:
                log.error(
                    "A callable entry point (%r) raised an " "unexpected error.",
                    ex,
                    exc_info=True,
                )
                continue
            workflows = [
                ExampleWorkflow(wf, package=None, distribution=ep.dist)
                for wf in workflows
            ]
