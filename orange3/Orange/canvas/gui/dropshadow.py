...
from AnyQt.QtCore import Qt, QPoint, QPointF, QRect, QRectF, QSize, QSizeF, QEvent
from AnyQt.QtCore import pyqtProperty as Property

CACHED_SHADOW_RECT_SIZE = (50, 50)


def render_drop_shadow_frame(
    pixmap, shadow_rect, shadow_color, offset, radius, rect_fill_color
):
    ...
    effect = QGraphicsDropShadowEffect(
        color=shadow_color, blurRadius=radius, offset=offset
    )
...
class DropShadowFrame(QWidget):
    ...
    color_ = Property(
        QColor, fget=color, fset=setColor, designable=True, doc="Drop shadow color"
    )
    ...
    def setWidget(self, widget):
        ...
        if self.__widget:
            ...
            while not (
                isinstance(parent, (QAbstractScrollArea, QToolBar)) or parent.isWindow()
            ):
    ...
    def paintEvent(self, event):
        ...
        source_rects = self.__shadowPixmapFragments(pixmap_rect, pixmap_shadow_rect)
    ...
    def __updatePixmap(self):
        ...
        # Size of the pixmap.
        pixmap_size = QSize(
            rect_size.width() + left + right, rect_size.height() + top + bottom
        )
    ...
    def __shadowPixmapFragments(self, pixmap_rect, shadow_rect):
        ...
        left = QRectF(
            pixmap_rect.topLeft() + QPointF(0.0, s_top), shadow_rect.bottomLeft()
        )
        return [
            top_left,
            top,
