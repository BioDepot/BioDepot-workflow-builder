...
class TestEntropyMDL(TestCase):
    ...
    def test_entropy_constant(self):
        X = np.zeros((100, 1))
        domain = Domain(
            [ContinuousVariable("v1")], [DiscreteVariable("c1", values=["1"])]
        )
    ...
    def test_entropy(self):
        X = np.array([0] * 25 + [1] * 25 + [2] * 25 + [3] * 25).reshape((100, 1))
...
class TestDiscretizer(TestCase):
    ...
    def test_create_discretized_var(self):
        dvar = discretize.Discretizer.create_discretized_var(self.var, [1, 2, 3])
        self.assertIsInstance(dvar.compute_value, discretize.Discretizer)
    ...
    def test_create_discretized_var_formatting(self):
        ...
        dvar = discretize.Discretizer.create_discretized_var(self.var, [10])
        self.assertEqual(dvar.values, ["< 10", "≥ 10"])

        dvar = discretize.Discretizer.create_discretized_var(self.var, [10.1234])
        self.assertEqual(dvar.values, ["< 10.1", "≥ 10.1"])

        self.var.number_of_decimals = 3

        dvar = discretize.Discretizer.create_discretized_var(self.var, [5, 10.1234])
    ...
    def test_discretizer_computation_sparse_no_points(self):
        dvar = discretize.Discretizer.create_discretized_var(self.var, [])
        X = sp.csr_matrix(np.array([0, 0.9, 1, 1.1, 1.9, 2, 2.5, 3, 3.5]))
    ...
    def test_remove_constant(self):
        ...
        self.assertNotEqual(
            len(table.domain.attributes), len(new_table.domain.attributes)
        )
    ...
    def test_keep_constant(self):
        ...
        new_table = discretize(table)
        self.assertEqual(len(table.domain.attributes), len(new_table.domain.attributes))

    def test_discretize_class(self):
        table = data.Table("iris")
        domain = table.domain
        regr_domain = data.Domain(
            domain.attributes[:3], [domain.attributes[3], domain.class_var]
        )
        table = data.Table.from_table(regr_domain, table)

        discretize = Discretize(remove_const=False)
        new_table = discretize(table)
        self.assertIs(new_table.domain.class_vars[0], new_table.domain.class_vars[0])
        self.assertIs(new_table.domain.class_vars[1], new_table.domain.class_vars[1])
...
class TestDiscretizeTable(TestCase):
    ...
    def test_fixed(self):
        dom = discretize.DomainDiscretizer(
            method=discretize.EqualWidth(n=2), fixed={"Feature 2": [1, 11]}
        )
