...
class TestInstance(unittest.TestCase):
    ...
    def create_domain(self, attributes=(), classes=(), metas=()):
        attr_vars = [
            ContinuousVariable(name=a) if isinstance(a, str) else a for a in attributes
        ]
        class_vars = [
            ContinuousVariable(name=c) if isinstance(c, str) else c for c in classes
        ]
        meta_vars = [
            DiscreteVariable(name=m, values=map(str, range(5)))
            if isinstance(m, str)
            else m
            for m in metas
        ]
    ...
    def test_init_xym_no_data(self):
        ...
        assert_array_nanequal(
            inst._metas, np.array([var.Unknown for var in domain.metas], dtype=object)
        )
    ...
    def test_init_xym(self):
        ...
        for val in (
            np.array([42, "M", "B", "X", 43, "Foo"], dtype=object),
            [42, "M", "B", "X", 43, "Foo"],
        ):
    ...
    def test_init_inst(self):
        ...
        self.assertEqual(inst2._y[0], 1)
        assert_array_nanequal(inst2._metas, np.array([0, Unknown, 42], dtype=object))
    ...
    def test_get_item(self):
        ...
        vals = [42, "M", "B", "X", 43, "Foo"]
        inst = Instance(domain, vals)
        for idx_int, idx_name, idx_var, value in (
            (0, "x", domain[0], 42),
            (1, "g", domain[1], "M"),
    ...
    def test_str(self):
        ...
        self.assertEqual(
            str(inst),
            "[{}]".format(
                ", ".join("{:.3f}".format(x) for x in range(len(self.attributes)))
            ),
        )
        ...
        self.assertEqual(
            str(inst),
            "[{}]".format(
                ", ".join("{}".format(x) for x in range(len(self.attributes)))
            ),
        )
