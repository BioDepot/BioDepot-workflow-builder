...
class TestUtil(unittest.TestCase):
    def test_export_globals(self):
        self.assertEqual(
            sorted(export_globals(globals(), __name__)), ["SOMETHING", "TestUtil"]
        )
    ...
    def test_vstack(self):
        numpy = np.array([[1.0, 2.0], [3.0, 4.0]])
        csr = sp.csr_matrix(numpy)
        csc = sp.csc_matrix(numpy)

        self.assertCorrectArrayType(
            vstack([numpy, numpy]), shape=(4, 2), sparsity="dense"
        )
        self.assertCorrectArrayType(
            vstack([csr, numpy]), shape=(4, 2), sparsity="sparse"
        )
        self.assertCorrectArrayType(
            vstack([numpy, csc]), shape=(4, 2), sparsity="sparse"
        )
        self.assertCorrectArrayType(vstack([csc, csr]), shape=(4, 2), sparsity="sparse")
    ...
    def test_hstack(self):
        ...
        self.assertCorrectArrayType(
            hstack([numpy, numpy]), shape=(2, 4), sparsity="dense"
        )
        self.assertCorrectArrayType(
            hstack([csr, numpy]), shape=(2, 4), sparsity="sparse"
        )
        self.assertCorrectArrayType(
            hstack([numpy, csc]), shape=(2, 4), sparsity="sparse"
        )
        self.assertCorrectArrayType(hstack([csc, csr]), shape=(2, 4), sparsity="sparse")
    ...
    @unittest.skipUnless(
        os.environ.get("ORANGE_DEPRECATIONS_ERROR"), "ORANGE_DEPRECATIONS_ERROR not set"
    )
