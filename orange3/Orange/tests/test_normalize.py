...
class TestNormalizer(unittest.TestCase):
    def compare_tables(self, dataNorm, solution):
        ...
        self.assertEqual(
            [attr.name for attr in dataNorm.domain.attributes],
            ["c1", "c2", "d1", "d2", "n1", "n2", "c3", "d3"],
        )
        self.assertEqual(
            [attr.name for attr in dataNorm.domain.class_vars], ["cl1", "cl2"]
        )
    ...
    def test_normalize_transform_by_sd(self):
        normalizer = Normalize(
            zero_based=False, norm_type=Normalize.NormalizeBySD, transform_class=False
        )
        data_norm = normalizer(self.data)
        solution = [
            [0.0, 1.225, "a", "a", "?", "a", 1.225, "a", "a", 2],
    ...
    def test_normalize_transform_class(self):
        normalizer = Normalize(
            zero_based=True, norm_type=Normalize.NormalizeBySD, transform_class=True
        )
        data_norm = normalizer(self.data)
        solution = [
            [0.0, 1.225, "a", "a", "?", "a", 1.225, "a", "a", 1.225],
            [0.0, -1.225, "a", "b", -1.0, "?", 0.0, "b", "b", -1.225],
            [0.0, 0.0, "a", "b", 1.0, "b", -1.225, "c", "c", 0.0],
        ]
        self.compare_tables(data_norm, solution)

    def test_normalize_transform_by_span(self):
        normalizer = Normalize(
            zero_based=False, norm_type=Normalize.NormalizeBySpan, transform_class=False
        )
        data_norm = normalizer(self.data)
        solution = [
            [0.0, 1.0, "a", "a", "?", "a", 1.0, "a", "a", 2.0],
            [0.0, -1.0, "a", "b", -1.0, "?", 0.0, "b", "b", 0.0],
            [0.0, 0.0, "a", "b", 1.0, "b", -1.0, "c", "c", 1.0],
        ]
        self.compare_tables(data_norm, solution)

    def test_normalize_transform_by_span_zero(self):
        normalizer = Normalize(
            zero_based=True, norm_type=Normalize.NormalizeBySpan, transform_class=False
        )
        data_norm = normalizer(self.data)
        solution = [
            [0.0, 1.0, "a", "a", "?", "a", 1.0, "a", "a", 2.0],
            [0.0, 0.0, "a", "b", 0.0, "?", 0.5, "b", "b", 0.0],
            [0.0, 0.5, "a", "b", 1.0, "b", 0.0, "c", "c", 1.0],
        ]
        self.compare_tables(data_norm, solution)

    def test_normalize_transform_by_span_class(self):
        normalizer = Normalize(
            zero_based=False, norm_type=Normalize.NormalizeBySpan, transform_class=True
        )
        data_norm = normalizer(self.data)
        solution = [
    ...
    def test_normalize_transform_by_span_zero_class(self):
        normalizer = Normalize(
            zero_based=True, norm_type=Normalize.NormalizeBySpan, transform_class=True
        )
        data_norm = normalizer(self.data)
        solution = [
            [0.0, 1.0, "a", "a", "?", "a", 1.0, "a", "a", 1.0],
            [0.0, 0.0, "a", "b", 0.0, "?", 0.5, "b", "b", 0.0],
            [0.0, 0.5, "a", "b", 1.0, "b", 0.0, "c", "c", 0.5],
        ]
        self.compare_tables(data_norm, solution)
    ...
    def test_normalize_sparse(self):
        ...
        # pylint: disable=bad-whitespace
        solution = sp.csr_matrix(np.array([[0, -1, -1], [0, 1, 1]]))
        ...
        data.X = sp.csr_matrix(np.array([[0, 0, 0], [0, 1, 3], [0, 2, 4]]))
