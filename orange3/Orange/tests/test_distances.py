...
def tables_equal(tab1, tab2):
    ...
    return tab1 == tab2 or np.all([i == j for i, j in zip(tab1, tab2)])  # catches None
    ...
    def test_from_file(self):
        with named_file(
            """3 axis=0 asymmetric col_labels row_labels
                ...
                eve	9.01	2.34	5.67
                frank	8.90	1.23	4.56"""
        ) as name:
            m = DistMatrix.from_file(name)
            np.testing.assert_almost_equal(
                m,
                np.array([[0.12, 3.45, 6.78], [9.01, 2.34, 5.67], [8.90, 1.23, 4.56]]),
            )
            self.assertIsInstance(m.row_items, Table)
            self.assertIsInstance(m.col_items, Table)
            self.assertEqual([e.metas[0] for e in m.col_items], ["ann", "bert", "chad"])
            self.assertEqual(
                [e.metas[0] for e in m.row_items], ["danny", "eve", "frank"]
            )
        ...
        with named_file(
            ...
            self.assertIsNone(m.col_items)
            self.assertEqual(
                [e.metas[0] for e in m.row_items], ["starič", "aleš", "anže"]
            )
            self.assertEqual(m.axis, 1)
        ...
        assertErrorMsg("axis=1\n1\t3\n4", "distance file must begin with dimension")
        assertErrorMsg(
            "3 col_labels\na\tb\n1\n\2\n3", "mismatching number of column labels"
        )
        assertErrorMsg(
            "3 col_labels\na\tb\tc\td\n1\n\2\n3", "mismatching number of column labels"
        )
        ...
        assertErrorMsg(
            "2 row_labels\na\t1\t2\t3\nb\t5", "too many columns in matrix row 'a'"
        )
        ...
        assertErrorMsg(
            "2 row_labels\na\t1\nb\tasd", "invalid element at row 'b', column 1"
        )
        assertErrorMsg(
...
class TestEuclidean(TestCase):
    ...
    def test_euclidean_distance_one_example(self):
        ...
        np.testing.assert_almost_equal(
            self.dist(self.iris[0], self.iris[0]), np.array([[0]])
        )
        np.testing.assert_almost_equal(self.dist(self.iris[0], axis=1), np.array([[0]]))
        np.testing.assert_almost_equal(
            self.dist(self.iris[0], self.iris[0], axis=1), np.array([[0]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[0], self.iris[1]), np.array([[0.53851648071346281]])
        )
        np.testing.assert_almost_equal(
    ...
    def test_euclidean_distance_many_examples(self):
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2]), np.array([[0.0, 0.53851648], [0.53851648, 0.0]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2], axis=0),
            np.array(
                [
                    [0.0, 2.48394847, 5.09313263, 6.78969808],
                    [2.48394847, 0.0, 2.64007576, 4.327817],
                    [5.09313263, 2.64007576, 0.0, 1.69705627],
                    [6.78969808, 4.327817, 1.69705627, 0.0],
                ]
            ),
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[2], self.iris[:3]), np.array([[0.50990195, 0.3, 0.0]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2], self.iris[3]),
            np.array([[0.64807407], [0.33166248]]),
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2], self.iris[:3]),
            np.array([[0.0, 0.53851648, 0.50990195], [0.53851648, 0.0, 0.3]]),
        )
    ...
    def test_euclidean_distance_sparse(self):
        np.testing.assert_almost_equal(
            ...
            np.array(
                [
                    [0.0, 3.74165739, 6.164414],
                    [3.74165739, 0.0, 4.47213595],
                    [6.164414, 4.47213595, 0.0],
                ]
            ),
        )
        np.testing.assert_almost_equal(
            ...
            np.array(
                [
                    [0.0, 4.12310563, 3.31662479],
                    [4.12310563, 0.0, 6.164414],
                    [3.31662479, 6.164414, 0.0],
                ]
            ),
        )
        np.testing.assert_almost_equal(
            self.dist(self.sparse[:2]), np.array([[0.0, 3.74165739], [3.74165739, 0.0]])
        )

    def test_euclidean_distance_numpy(self):
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2].X), np.array([[0.0, 0.53851648], [0.53851648, 0.0]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[2].x, self.iris[:3].X),
            np.array([[0.50990195, 0.3, 0.0]]),
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2].X, self.iris[3].x),
            np.array([[0.64807407], [0.33166248]]),
...
class TestManhattan(TestCase):
    ...
    def test_manhattan_distance_one_example(self):
        ...
        np.testing.assert_almost_equal(
            self.dist(self.iris[0], self.iris[1]), np.array([[0.7]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[0], self.iris[1], axis=1), np.array([[0.7]])
        )

    def test_manhattan_distance_many_examples(self):
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2]), np.array([[0.0, 0.7], [0.7, 0.0]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2], axis=0),
            np.array(
        ...
        np.testing.assert_almost_equal(
            self.dist(self.iris[2], self.iris[:3]), np.array([[0.8, 0.5, 0.0]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2], self.iris[3]), np.array([[1.0], [0.5]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2], self.iris[:3]),
            np.array([[0.0, 0.7, 0.8], [0.7, 0.0, 0.5]]),
        )

    def test_manhattan_distance_sparse(self):
        np.testing.assert_almost_equal(
            self.dist(self.sparse),
            np.array([[0.0, 6.0, 10.0], [6.0, 0.0, 6.0], [10.0, 6.0, 0.0]]),
        )
        np.testing.assert_almost_equal(
            self.dist(self.sparse, axis=0),
            np.array([[0.0, 5.0, 5.0], [5.0, 0.0, 10.0], [5.0, 10.0, 0.0]]),
        )
        np.testing.assert_almost_equal(
            self.dist(self.sparse[:2]), np.array([[0.0, 6.0], [6.0, 0.0]])
        )

    def test_manhattan_distance_numpy(self):
        np.testing.assert_almost_equal(
            self.dist(self.iris[0].x, self.iris[1].x, axis=1), np.array([[0.7]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2].X), np.array([[0.0, 0.7], [0.7, 0.0]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[2].x, self.iris[:3].X), np.array([[0.8, 0.5, 0.0]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2].X, self.iris[3].x), np.array([[1.0], [0.5]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2].X, self.iris[:3].X),
            np.array([[0.0, 0.7, 0.8], [0.7, 0.0, 0.5]]),
...
class TestCosine(TestCase):
    ...
    def test_cosine_distance_one_example(self):
        ...
        np.testing.assert_almost_equal(
            self.dist(self.iris[0], self.iris[1]), np.array([[0.00142084]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[0], self.iris[1], axis=1), np.array([[0.00142084]])
        )

    def test_cosine_distance_many_examples(self):
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2]),
            np.array([[0.0, 1.42083650e-03], [1.42083650e-03, 0.0]]),
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2], axis=0),
            np.array(
                [
                    [0.0, 1.61124231e-03, 1.99940020e-04, 1.99940020e-04],
        ...
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2], self.iris[3]),
            np.array([[0.00089939], [0.00120607]]),
        )
        np.testing.assert_almost_equal(
            self.dist(self.iris[:2], self.iris[:3]),
            np.array(
                [
                    [0.0, 1.42083650e-03, 1.26527175e-05],
    ...
    def test_cosine_distance_sparse(self):
        np.testing.assert_almost_equal(
            ...
            np.array(
                [
                    [0.0, 1.00000000e00, 7.20627882e-01],
                    [1.00000000e00, 0.0, 2.19131191e-01],
        ...
        np.testing.assert_almost_equal(
            ...
            np.array(
                [
                    [0.0, 7.57464375e-01, 1.68109669e-01],
                    [7.57464375e-01, 0.0, 1.00000000e00],
                    [1.68109669e-01, 1.00000000e00, 0.0],
                ]
            ),
        )
        np.testing.assert_almost_equal(
            self.dist(self.sparse[:2]),
            np.array([[0.0, 1.00000000e00], [1.00000000e00, 0.0]]),
        )

    def test_cosine_distance_numpy(self):
        np.testing.assert_almost_equal(
            self.dist(self.iris[0].x, self.iris[1].x, axis=1), np.array([[0.00142084]])
        )
        np.testing.assert_almost_equal(
...
class TestJaccard(TestCase):
    ...
    def test_jaccard_distance_one_example(self):
        ...
        np.testing.assert_almost_equal(
            self.dist(self.titanic[0], self.titanic[0]), np.array([[0]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.titanic[0], axis=1), np.array([[0]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.titanic[0], self.titanic[0], axis=1), np.array([[0]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.titanic[0], self.titanic[2]), np.array([[0.5]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.titanic[0], self.titanic[2], axis=1), np.array([[0.5]])
        )
    ...
    def test_jaccard_distance_many_examples(self):
        ...
        np.testing.assert_almost_equal(
            ...
            np.array([[0.0, 1.0, 0.5], [1.0, 0.0, 1.0], [0.5, 1.0, 0.0]]),
        )
        np.testing.assert_almost_equal(
            self.dist(self.titanic[2], self.titanic[:3]), np.array([[0.5, 0.5, 0.0]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.titanic[:2], self.titanic[3]), np.array([[0.5], [0.5]])
        )
        np.testing.assert_almost_equal(
    ...
    def test_jaccard_distance_numpy(self):
        np.testing.assert_almost_equal(
            self.dist(self.titanic[0].x, self.titanic[2].x, axis=1), np.array([[0.5]])
        )
        np.testing.assert_almost_equal(
        ...
        np.testing.assert_almost_equal(
            self.dist(self.titanic[:2].X, self.titanic[3].x), np.array([[0.5], [0.5]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.titanic[:2].X, self.titanic[:3].X),
            np.array([[0.0, 0.0, 0.5], [0.0, 0.0, 0.5]]),
...
class TestSpearmanR(TestCase):
    ...
    def test_spearmanr_distance_one_example(self):
        ...
        np.testing.assert_almost_equal(
            self.dist(self.breast[0], axis=1), np.array([[0]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.breast[0], self.breast[0]), np.array([[0]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.breast[0], self.breast[0], axis=1), np.array([[0]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.breast[0], self.breast[1]), np.array([[0.5083333333333333]])
        )
        np.testing.assert_almost_equal(
    ...
    def test_spearmanr_distance_many_examples(self):
        np.testing.assert_almost_equal(
            ...
            np.array([[0.0, 0.5083333333333333], [0.5083333333333333, 0.0]]),
        )
        np.testing.assert_almost_equal(
            self.dist(self.breast[:4]),
            np.array(
                [
                    [0.0, 0.50833333, 0.075, 0.61666667],
                    [0.50833333, 0.0, 0.38333333, 0.53333333],
                    [0.075, 0.38333333, 0.0, 0.63333333],
                    [0.61666667, 0.53333333, 0.63333333, 0.0],
                ]
            ),
        )
        np.testing.assert_almost_equal(
            ...
            np.array(
                [
                    [0.0, 0.25, 0.0, 0.25, 0.25, 0.25, 0.75, 0.25, 0.25],
                    [0.25, 0.0, 0.25, 0.0, 0.0, 0.0, 0.25, 0.0, 0.75],
                    [0.0, 0.25, 0.0, 0.25, 0.25, 0.25, 0.75, 0.25, 0.25],
                    [0.25, 0.0, 0.25, 0.0, 0.0, 0.0, 0.25, 0.0, 0.75],
                    [0.25, 0.0, 0.25, 0.0, 0.0, 0.0, 0.25, 0.0, 0.75],
                    [0.25, 0.0, 0.25, 0.0, 0.0, 0.0, 0.25, 0.0, 0.75],
                    [0.75, 0.25, 0.75, 0.25, 0.25, 0.25, 0.0, 0.25, 1.0],
                    [0.25, 0.0, 0.25, 0.0, 0.0, 0.0, 0.25, 0.0, 0.75],
                    [0.25, 0.75, 0.25, 0.75, 0.75, 0.75, 1.0, 0.75, 0.0],
                ]
            ),
        )
        np.testing.assert_almost_equal(
    ...
    def test_spearmanr2(self):
        ...
        np.testing.assert_almost_equal(_spearmanr2(X1, X2, axis=1), expected, decimal=9)

        expected = scipy.stats.spearmanr(X1, X2, axis=0)[0][:m, m:]
        np.testing.assert_almost_equal(_spearmanr2(X1, X2, axis=0), expected, decimal=9)
...
class TestSpearmanRAbsolute(TestCase):
    ...
    def test_spearmanrabsolute_distance_one_example(self):
        ...
        np.testing.assert_almost_equal(
            self.dist(self.breast[0], self.breast[1]), np.array([[0.49166666666666664]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.breast[0], self.breast[1], axis=1),
    ...
    def test_spearmanrabsolute_distance_many_examples(self):
        np.testing.assert_almost_equal(
            self.dist(self.breast[:2]), np.array([[0.0, 0.49166667], [0.49166667, 0.0]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.breast[:3], axis=0),
            np.array(
                [
                    [0.0, 0.25, 0.0, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25],
        ...
        np.testing.assert_almost_equal(
            ...
            np.array(
                [
                    [0.0, 0.49166667, 0.075, 0.38333333],
                    [0.49166667, 0.0, 0.38333333, 0.46666667],
                    [0.075, 0.38333333, 0.0, 0.36666667],
                ]
            ),
        )
        np.testing.assert_almost_equal(
            self.dist(self.breast[3], self.breast[:4]),
            np.array([[0.3833333, 0.4666667, 0.3666667, 0.0]]),
        )
        np.testing.assert_almost_equal(
    ...
    def test_spearmanrabsolute_distance_numpy(self):
        ...
        np.testing.assert_almost_equal(
            self.dist(self.breast[:2].X),
            np.array([[0.0, 0.49166667], [0.49166667, 0.0]]),
        )
        np.testing.assert_almost_equal(
...
class TestPearsonR(TestCase):
    ...
    def test_pearsonr_distance_one_example(self):
        ...
        np.testing.assert_almost_equal(
            self.dist(self.breast[0], self.breast[1]), np.array([[0.48462293898088876]])
        )
        np.testing.assert_almost_equal(
            self.dist(self.breast[0], self.breast[1], axis=1),
    ...
    def test_pearsonr_distance_many_examples(self):
        np.testing.assert_almost_equal(
            self.dist(self.breast[:2]), np.array([[0.0, 0.48462294], [0.48462294, 0.0]])
        )
        ...
        np.testing.assert_almost_equal(
            ...
            np.array(
                [
                    [0.0, 0.48462294, 0.10133593, 0.5016744],
                    [0.48462294, 0.0, 0.32783865, 0.57317387],
                    [0.10133593, 0.32783865, 0.0, 0.63789635],
                ]
            ),
        )
        np.testing.assert_almost_equal(
            self.dist(self.breast[2], self.breast[:3]),
            np.array([[0.10133593, 0.32783865, 0.0]]),
        )
        np.testing.assert_almost_equal(
            self.dist(self.breast[:3], self.breast[2]),
            np.array([[0.10133593], [0.32783865], [0.0]]),
    ...
    def test_pearsonr_distance_numpy(self):
        ...
        np.testing.assert_almost_equal(
            self.dist(self.breast[:2].X),
            np.array([[0.0, 0.48462294], [0.48462294, 0.0]]),
        )
        np.testing.assert_almost_equal(
            self.dist(self.breast[2].x, self.breast[:3].X),
            np.array([[0.10133593, 0.32783865, 0.0]]),
    ...
    def test_corrcoef2(self):
        ...
        np.testing.assert_almost_equal(_corrcoef2(X1, X2, axis=1), expected, decimal=9)

        expected = np.corrcoef(X1, X2, rowvar=False)[:m, m:]
        np.testing.assert_almost_equal(_corrcoef2(X1, X2, axis=0), expected, decimal=9)
...
class TestPearsonRAbsolute(TestCase):
    ...
    def test_pearsonrabsolute_distance_many_examples(self):
        ...
        np.testing.assert_almost_equal(
            ...
            np.array(
                [
                    [0.0, 0.48462294, 0.10133593, 0.4983256],
                    [0.48462294, 0.0, 0.32783865, 0.42682613],
                    [0.10133593, 0.32783865, 0.0, 0.36210365],
                ]
            ),
        )
        np.testing.assert_almost_equal(
...
class TestDistances(TestCase):
    ...
    def test_preprocess(self):
        domain = Domain(
            [ContinuousVariable("c"), DiscreteVariable("d", values=["a", "b"])],
