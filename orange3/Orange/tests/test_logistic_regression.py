...
class TestLogisticRegressionLearner(unittest.TestCase):
    ...
    def test_LogisticRegressionNormalization(self):
        ...
        new_domain = Domain(
            new_attrs, self.iris.domain.class_vars, self.iris.domain.metas
        )
        new_table = np.hstack(
            (1000000 * np.random.random((self.iris.X.shape[0], 1)), self.iris)
        )
        ...
        model = lr_norm(table)
        y = np.argmax(np.dot(table.X, model.coefficients.T) + model.intercept, axis=1)
    ...
    def test_learner_scorer_multiclass(self):
        ...
        self.assertEqual(
            scores.shape, (len(self.zoo.domain.class_var.values), len(attr))
        )
